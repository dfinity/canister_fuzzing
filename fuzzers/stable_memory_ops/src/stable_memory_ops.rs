...
const EXECUTION_DIR: &str = "fuzzers/stable_memory_ops";
...
fn read_canister_bytes() -> Vec<u8> {
    let wasm_path = std::path::PathBuf::from(std::env::var("STABLE_MEMORY_WASM_PATH").unwrap());
...
pub fn run() {
    let mut harness = |input: &BytesInput| {
        ...
        let result =
            test.execute_ingress(canister_id, "stable_memory_ops", (*input).clone().into());
